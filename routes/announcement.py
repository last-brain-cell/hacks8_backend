...
async def get_nearby_users_for_disaster(
        disaster_id: str = Query(..., description="The ID of the disaster for which to find nearby users")):
    ...
    nearby_users = await UserLocationUpdate.find(
        Near(UserLocationUpdate.location, disaster.location[0], disaster.location[1], max_distance=max_distance)
    )
...
async def send_announcement_to_nearby_users(announcement: Announcement):
    ...
    max_distance = 100
    nearby_users = await UserLocationUpdate.find(
        Near(UserLocationUpdate.location, disaster_location[0], disaster_location[1], max_distance=max_distance)
    )
